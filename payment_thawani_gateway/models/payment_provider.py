...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[('thawani2', "Thawani2")],
        ondelete={'thawani2': 'set default'}
    )
    ...
    def _thawani_make_request(self, endpoint, json=None, method='POST'):
        ...
        try:
            ...
            raise ValidationError("Thawani2: " + _(
                "Thawani2 API returned the following response: %s\n%s",
                str(response),
                "'"+str(response.text)+"'"
            ))
            ...
            raise ValidationError(
                "Thawani2: " + _("Could not establish the connection to the API.")
            )
