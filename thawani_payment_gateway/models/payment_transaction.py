        ...
        for rec in order_line:
            if rec.price_unit > 0: 
                if rec.product_template_id.detailed_type!='service':
                    ...
                    _logger.info(f'cxxxcxc{sorted([i.company_rate for rec.currency_id.rate_ids])}')
                    ...
                    int((rec.price_unit * 1000 * 1.25)/int(sorted([i.company_rate for rec.currency_id.rate_ids])[0])) - int(((rec.price_unit * 1000*1.25)* dis )/100)  if rec.currency_id.name =='KWD' else int((rec.price_unit * 1000)/int([i.company_rate for rec.currency_id.rate_ids])[0]) - int(((rec.price_unit * 1000)* dis )/100),
                    }
